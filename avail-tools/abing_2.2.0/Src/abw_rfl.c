...
FILE *caf;
	...
	int	np;			/* sender's packet number */
	int	np1;			/* sender's P1packet number */
	int	np2;			/* sender's P2packet number */
	int	train;			/* packets in sequence */
	int	nrcv = 0;		/* number of packets rcvd */
	int 	i,lindex;
	int 	okfl=0;
	int 	sqfl=0;
	int 	cafl=0;
...
// backward compatibility (don't know what is there)
        ...
        lindex=np%(train);
		ptrec = &arec[lindex];
		ptrec->mirr.tv_sec = htonl ((u_long) tp->tv_sec);
		ptrec->mirr.tv_usec = htonl ((u_long) tp->tv_usec);
		ptrec->rnum = htonl ((u_long) nrcv);
