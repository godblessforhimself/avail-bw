...
//        dis_rtt=dDisp(tdt,nbunches,avr_rtt);
//        sig_rtt=sqrt(dis_rtt);
        min_rtt=dMin(tdt,nbunches);
        max_rtt=dMax(tdt,nbunches);
// printf ("\nRTTcalcul1: AV=%f, DISP=%f, SIGMA=%f\n",avr_rtt,dis_rtt,sig_rtt);
// printf ("RTTcalcul2: MIN=%f,MAX=%f, S=%d R=%d\n",min_rtt,max_rtt,npackets/2,nbunches+1);
...
// count start 1 (PP 1,2 for second is valid i =4 
	for (i=4;i<=xpackets;i++) {
		...
		tdt[jx]=rtt;
//  printf ("Rtt/Rttx[%d,%d]=%f/%f, from (%f - %f -%f)\n",i,jx,rtt,rttx,arrR[i],arrS[i],ppdiffm);
//  		if (jx > 0) {printf ("[%d,%d]=%6.3f, ",i,jx,rtt);} else { printf ("RTTs: [%d,%d]=%6.3f, ",i,jx,rtt);}
		jx++;
		nbunches=jx;
			}
		}
	}
	if (nbunches > 1) {vbunches=nbunches+1;} else {vbunches=nbunches;}  
//	printf("RCV %d packets WHY bunches=%d/%d i=%d,jx=%d,jy=%d] \n",xpackets,nbunches,vbunches,i,jx,jy);
/***************************/
