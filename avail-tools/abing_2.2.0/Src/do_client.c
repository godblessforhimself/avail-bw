...
int Do_Client_Work(char * serverName, unsigned short int serverPort, int rpt_cnt)
{
...
//        printf("I am parent and I will do receiving\n");
  ...
  while (1) {  
        	...
        	np = ntohl ((u_long) rrec->num);
        	rnum = ntohl ((u_long) rrec->rnum);
        	ppseq = ntohl ((u_long) rrec->ppseq);
		rcnt++;
//    		printf("Received[%d] %d bytes <%s> fromServer: %s:%d %d,%d,%d\n",rcnt,incnt,ptext,inet_ntoa(src.sin_addr),htons(src.sin_port),np,rnum,ppseq);    /* Print the echoed arg */
        	if (np) {
                	rpackets++;
                	endcnt=0;
                	arr_hs_snd[np]= (double) ntohl((u_long) rrec->snd.tv_sec);
                	arr_hus_snd[np]=(double) ntohl((u_long) rrec->snd.tv_usec);
                	arr_hs_mir[np]= (double) ntohl((u_long) rrec->mirr.tv_sec);
                	arr_hus_mir[np]=(double) ntohl((u_long) rrec->mirr.tv_usec);
