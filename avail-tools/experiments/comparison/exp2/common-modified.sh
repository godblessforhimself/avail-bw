...
runSpruce(){
	...
	for ((i=1;i<=100;i++));do
		...
		sudo dagconvert -T erf:pcap -i /tmp/dag/dagsnap.erf -b "${bpf}" -f a -o /tmp/dag/dagsnap.pcap 1>/dev/null 2>&1
		tshark -r /tmp/dag/dagsnap.pcap -Tfields -e frame.time_epoch -e frame.len 1>${1}/dagsnap.txt
		cat ${1}/dagsnap.txt >> ${1}/dag.in
		echo "" >> ${1}/dag.in
		sudo dagconvert -T erf:pcap -i /tmp/dag/dagsnap.erf -b "${bpf}" -f c -o /tmp/dag/dagsnap.pcap 1>/dev/null 2>&1
		tshark -r /tmp/dag/dagsnap.pcap -Tfields -e frame.time_epoch -e frame.len 1>${1}/dagsnap.txt
		cat ${1}/dagsnap.txt >> ${1}/dag.out
		echo "" >> ${1}/dag.out
	done
