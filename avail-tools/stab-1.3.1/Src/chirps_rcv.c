...
inline void update_info(u_int32_t nc,u_int32_t np,u_int32_t ttl,double snd_time,double rcv_time,u_int32_t pkt_req_num,int good_jumbo_pkt)
{
  ...
  packet_info[num_pkts_in_info].ttl=ttl;
...
{  
  ...
  u_int32_t prev_chirp=0,prev_pkt_num=0,prev_pkt_chirp=0,ttl=0,prev_pkt_ttl=0,good_pkt_ttl=0;/* previous chirp number */
 ...
 /* this timer is set just in case the sender goes down and we are not
  ...
  while (1) {
	...
	if (cmptr->cmsg_level != SOL_SOCKET)
	  {
	    perror("stab_rcv: control not SOL_SOCKET\n");
	    exit(0);
	  }
	
	if (cmptr->cmsg_type != SCM_TIMESTAMP)
	  {
	    perror("stab_rcv: type not SCM_TIMESTAMP\n");
	    exit(0);
	  }
    ...
    np = ntohl (udprecord->num);
    nc = ntohl (udprecord->chirp_num);

    ttl = ntohl (udprecord->big_pkt_ttl);
    ...
    if (!np) {
      (void) fprintf (stderr,
		      "\nstab_rcv: Finished servicing this client\n");
      break;
    }
	...
	if(jumbo>1)
	  {
		...
		if (good_pkt_num==np-1)
		  {
		    update_info(good_chirp_num,good_pkt_num,good_pkt_ttl,good_pkt_snd_time,good_pkt_rcv_time,ntohl(udprecord->request_num),1);
		  }
		else
		  if (prev_pkt_num==np-1)/*record last packet of previous jumbo*/
		    {
		      update_info(prev_pkt_chirp,prev_pkt_num,prev_pkt_ttl,prev_pkt_snd_time,prev_pkt_rcv_time,ntohl(udprecord->request_num),0);
	    ...
	    if (np==num_interarrival[ttl_index]+1 && prev_pkt_num<num_interarrival[ttl_index]+1)
	      update_info(nc,np,ttl,cur_pkt_snd_time,cur_pkt_rcv_time,ntohl(udprecord->request_num),1);
