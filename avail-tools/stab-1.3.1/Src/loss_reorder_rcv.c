...
void check_reorder_loss()
{
 ...
 unsigned long nc,np;
    ...
    if(debug) 
	{
	  fprintf(stderr,"In loss reorder, chirps_per_write=%d\n",chirps_per_write);
	}
  /* initializing */
  memset(chirp_info, 0, (int) chirps_per_write*2 * sizeof(struct chirprecord));
   ...
   if(debug) 
	{
	  fprintf(stderr,"Finished initializing chirp_info\n");
	}
  ...
  for (count=1;count<num_pkts_in_info;count++)
      ...
      if (nc>=first_chirp)
	{
	  ...
	  if (packet_info[count].request_num<request_num)
	    {
	      chirp_info[nc-first_chirp].reorder=2;
	      if(debug)
		fprintf(stderr,"request num low, pktreqno=%d,actualreq=%d\n",packet_info[count].request_num,request_num);
	    }

	  /*record current sender request number */	  
