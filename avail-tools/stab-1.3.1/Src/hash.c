...
/* SECURITY: In this implementation stab_snd will be on
continuously and wait for stab_rcv to initiate the connection.
In addition stab_rcv acts as a master telling stab_snd
at what rates to send packets etc. 

As the first step towards making stab more secure we
...
u_int32_t gen_crc_rcv2snd(struct control_rcv2snd *pkt)
{
  ...
  if (debug) fprintf(stderr,"gen rcv2snd\n");
...
int check_crc_rcv2snd(struct control_rcv2snd *pkt)
{
  ...
  u_int32_t crc;
  if (debug) fprintf(stderr,"check rcv2snd\n");
...
{
  u_int32_t cur_element,crc;
  if (debug) fprintf(stderr,"gen snd2rcv\n");
  ...
  cur_element=ntohl(pkt->chal_no);
  crc=hash(crc,cur_element);
  if (debug) fprintf(stderr,"fin gen snd2rcv\n");
  return(crc);
}
...
int check_crc_snd2rcv(struct udprecord *pkt)
{
  ...
  u_int32_t crc;
  if (debug) fprintf(stderr,"check snd2rcv\n");
