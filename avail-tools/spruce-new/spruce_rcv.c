...
spruce_dump(char * filename){
  ...
  for(i=0 ; i < spruce_next_empty && i < spruce_total ; i++ ){
    fprintf(out, "%d %d %d %d %d %d %lu.%06lu\n",
	   ...
	   (int) spruce_table[i].sendgap,
	         spruce_table[i].t.tv_sec,
	         spruce_table[i].t.tv_usec);
  }
  ...
  storedtime.tv_sec = 0;
	...
	storedsize = spruce_table[i].size;
	...
	if(spruce_table[i].seqno == expected){
	  ...
	  capacity = (storedsize * 8 * (long long) (1000000))
	    / storedgap;
	  // gout - gin
	  dr = rgap - ((spruce_table[i].size) * 8) / (capacity / 1e6);
	  dr = dr < 0 ? 0 : dr;
    ...
    fprintf(out, "unused %lu %lu\n", storedsendgap, storedtotpairs);
  }
...
void tcp_control_read(){
  ...
  case CONTROL_NOOP:
    ack = cmd | CONTROL_ACK;
    _unused=write(tcp_control_socket,&ack,sizeof ack);
    break;
  case CONTROL_EXIT:
    ack = cmd | CONTROL_ACK;
    _unused=write(tcp_control_socket,&ack,sizeof ack);
  ...
  case CONTROL_SPRUCE_OUTPUT:
    ack = cmd | CONTROL_ACK;
    {
      ...
      rate = htonl(rate);
      _unused=write(tcp_control_socket, &ack, sizeof ack);
      _unused=write(tcp_control_socket, &rate, sizeof(long));
    }
