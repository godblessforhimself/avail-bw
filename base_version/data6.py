...
def linear_log2file(f,packet_size,data):
    ...
    for i in yset_test:
        v=xtest[np.repeat(ytest==i,3),1]
        value_str=list_formatter(v*1e3,'{:.2f}')
        pred=(ymax-ymin)*(v-recv_min)/(recv_max-recv_min)+ymin
        pred_mean=np.mean(pred)
        pred_str=list_formatter(pred,'{:.2f}')
        f.write('load {:.1f} value {} pred {} pred_mean {:.2f}\n'.format(i, value_str, pred_str, pred_mean))
...
def get_data():
    #link100load0packetsize1500exp6.txt
    #link100load0packetsize1500exp6test.txt
    data_set=[]
    for packet_size in range(1500,9001,500):
        pattern_train=r'^link100load[0-9\.]+packetsize{}exp6\.txt'.format(packet_size)
        pattern_test=r'^link100load[0-9\.]+packetsize{}exp6test[0-9]*\.txt'.format(packet_size)
...
linear_log=open('temp/linear_log_exp6.txt','w')
param_linear={
...
def run_experiment(data_set):
    for i,packet_size in enumerate(range(1500,9001,500)):
        data=data_set[i]
        if len(data[0])==0:
            break
        ...
        for i,name in enumerate(method_list):
            print('{} use {:.2f} s, res {}\nres average {}'.format(name,r2[i],list_formatter(r1[i][0],'{:.2f}'),list_formatter(r1[i][1],'{:.2f}')))
        print('\n')
