#!/bin/bash
# author: tony
# exp7：包大小对准确率的影响100Mbps -> 改进exp6的test数据
# 1500,9000,1500 6
# continuous count 由探测包大小决定
# 训练 背景流量0-90 100轮
# 测试 背景流量0-90 0.3 10轮*3
...
TEST_TRAFFIC_RANGE=$(seq 0 0.3 90)

set_src_link(){
    BURST_SIZE=$(echo $1/1000|python3 -c "import math;print('{}kb'.format(math.ceil(eval(input()))+1))")
    sudo tc qdisc del dev enp61s0f1 root; sudo tc qdisc add dev enp61s0f1 root tbf rate 100mbit burst ${BURST_SIZE} limit 10000kb
}
set_router_link(){
...
get_data(){
    for PROBING_PACKET_SIZE in $(seq 1500 1500 9000); do
        ...
        CONTINUOUS_COUNT=$(echo 1500*100/${PROBING_PACKET_SIZE} | python3 -c "import math;print(math.ceil(eval(input())))")
        #train data
        ...
        for CROSS_TRAFFIC_RATE in ${TEST_TRAFFIC_RANGE}; do
            ...
            for TEST_I in $(seq 1 $TEST_REPEAT_COUNT); do
                clear_probing
                TS_FILENAME="/home/liqing/avail-bw/data/link100load${CROSS_TRAFFIC_RATE}packetsize${PROBING_PACKET_SIZE}exp${EXPERIMENT_COUNT}test${TEST_I}.txt"
                sleep 0.01s
